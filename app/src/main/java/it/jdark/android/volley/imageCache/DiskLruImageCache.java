...
import it.jdark.android.volley.BuildConfig;
...
public class DiskLruImageCache implements ImageCache {
    ...
    public void putBitmap(String key, Bitmap data) {
        ...
        try {
            ...
            if (writeBitmapToFile(data, editor)) {
                mDiskCache.flush();
                editor.commit();
                Log.d(TAG, "Cache Test Disk: image put on disk cache " + key);
            } else {
                editor.abort();
                Log.d(TAG, "Cache Test Disk: ERROR - put image on disk cache " + key);
            }
        } catch (IOException e) {
            Log.d(TAG, "Cache Test Disk: ERROR - put image on disk cache " + key);
            try {
    ...
    public Bitmap getBitmap(String key) {
        ...
        if (BuildConfig.DEBUG) {
            Log.d(TAG, bitmap == null ? "Cache Test Disk: " : "Cache Test Disk: image read from disk " + key);
        }
    ...
    public void clearCache() {
        Log.d(TAG, "Cache Test Disk: disk cache CLEARED");
        try {
            mDiskCache.delete();
        } catch (IOException e) {
            e.printStackTrace();
